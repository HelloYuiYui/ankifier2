...
# The user marks the part(s) to hide as -like this-.
#
...
def render_as_is(text: str, part_hints: list[str]) -> tuple[str, str]:
    """Return (plain_text, cloze_text) for an as-is source text.
    ...
    numbered c1, c2, ... in order of appearance; part_hints supplies each hint
    positionally, and a missing or empty hint yields a bare {{cN::text}} rather
    than an empty ::hint, which Anki renders as a stray "[...]" gap.
    """
    plain: list[str] = []
    cloze: list[str] = []
    pos = 0
...
def strip_markers(text: str) -> str:
    ...
    return render_as_is(text, [])[0]


def build_as_is_cloze(text: str, part_hints: list[str]) -> str:
    """Turn -...- markers into Anki clozes. See render_as_is."""
    return render_as_is(text, part_hints)[1]
